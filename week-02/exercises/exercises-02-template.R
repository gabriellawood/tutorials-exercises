...
round(water_temp_f, digits = 1)
	...
	number_measured <- length(fish_lengths)
	number_measured
	...
	cod_observations <- sum(survey_data$species == "cod")
	...
	mean(survey_data$length_cm[survey_data$caught == TRUE])
	...
	#
	# Response: Writing code from a data frame to get an answer became clearer 
	  #after doing it repeatedly.
